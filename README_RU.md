...
- Полноценный Git-клиент: Overview, Changes + просмотр diff + коммиты, ветки, история, граф коммитов, удалённые репозитории и вкладка GitHub (репозитории, pull request, issues)
...
Откройте панель Git комбинацией `Ctrl+Shift+G`. Раздел состоит из 7 секций, переключение —
**Влево / Вправо** (или `[` / `]`):

| Секция | Назначение |
|--------|-----------|
| Overview | Статус репозитория, текущая ветка, ahead/behind, URL удалённого репозитория, последний коммит, быстрые действия |
| Changes | Список файлов, просмотр diff, стейджинг, поля сообщения коммита, commit / commit & push |
| Branches | Создание, переименование, удаление, слияние, переключение веток |
| History | Список коммитов с панелью деталей; копирование хеша, ветка от коммита, revert, reset |
| Graph | ASCII-граф коммитов |
| Remotes | Fetch, push, pull, добавление/удаление удалённых репозиториев |
| GitHub | Подключение по personal access token, просмотр репозиториев, создание pull request и issues |

### Overview
| Клавиша | Действие |
|---------|----------|
| O | Открыть репозиторий на GitHub (браузер) |
| F | Fetch |
| P | Push |
| L | Pull |
| S | Stash изменений |
...
| Клавиша | Действие |
|---------|----------|
| Enter / Пробел / S / U | Застейджить / снять со стейджинга выбранный файл |
| A | Застейджить всё |
| Z | Снять со стейджинга всё |
| D | Отменить изменения файла (подтверждение) |
| Tab | Переключение между списком файлов и сообщением коммита |
| C | Commit |
| P | Commit & push |
| X | Переключение обычный / stacked diff |

### Branches
| Клавиша | Действие |
|---------|----------|
| Enter | Переключиться на ветку |
| N | Создать ветку |
| R | Переименовать ветку |
| D | Удалить ветку (подтверждение) |
| M | Слить ветку |
| F | Fetch all |
...
| Клавиша | Действие |
|---------|----------|
| C | Скопировать полный хеш коммита |
| B | Создать ветку от коммита |
| V | Откатить коммит (revert) |
| R | Reset до коммита |
| F5 | Перезагрузить историю |

### Graph / Remotes
- **Graph**: прокрутка Вверх/Вниз, R / F5 перезагрузить.
- **Remotes**: F fetch, P push, L pull, A добавить remote, D удалить (подтверждение), O открыть в браузере.

### GitHub
| Клавиша | Действие |
|---------|----------|
| C | Подключение / ввод personal access token (хранится в `%APPDATA%\KShell\github_token.txt`) |
| Вверх/Вниз | Выбор репозитория |
| O | Открыть репозиторий в браузере |
| N | Создать pull request |
| I | Создать issue |
| R / F5 | Обновить список репозиториев |

> **Примечание про Commit & Push.** «Push complete» означает, что локальная ветка
> отправлена в настроенный `origin` репозитория. Если на GitHub ничего не появилось —
> проверьте строку Remote в разделе Overview (или выполните `git remote -v`): локальный
> `origin` должен указывать на нужный GitHub-репозиторий. Если remote отсутствует, а на
> вкладке GitHub подключён аккаунт и выбран репозиторий, `origin` добавляется
> автоматически. Push из detached HEAD отклоняется с подсказкой вместо молчаливого
> «успеха».
